...
class IntegrationService {
  IntegrationService(this._integrationManager, this._connectorManager) {
    ...
    if (_integrationManager.isServiceConnected('google', 'tasks')) {
      final googleProvider = _integrationManager.getProvider('google')
          as GoogleIntegrationProvider?;
      if (googleProvider?.authClient != null) {
        _tasksService = GoogleTasksService(_integrationManager,
            _connectorManager, googleProvider!.authClient!);
      }
    }
  }
  ...
  bool get isAuthenticated =>
      _integrationManager.isProviderAuthenticated('google');

  /// Check if specific services are connected
  bool get isTasksConnected =>
      _integrationManager.isServiceConnected('google', 'tasks');
  bool get isCalendarConnected =>
      _integrationManager.isServiceConnected('google', 'calendar');
  bool get isGmailConnected =>
      _integrationManager.isServiceConnected('google', 'gmail');

  /// Get list of connected services
  Set<String> get connectedServices => _integrationManager
      .getConnectedServices('google')
      .map((s) => s.id)
      .toSet();

  /// Check if a specific service is connected
  bool isServiceConnected(String service) =>
      _integrationManager.isServiceConnected('google', service);
  ...
  /// Create a new task
  Future<Map<String, dynamic>> createTask(String title,
      {String? notes, DateTime? due}) async {
    try {
      // Ensure we have valid authentication before making API calls
      if (!_integrationManager.isProviderAuthenticated('google')) {
        throw Exception(
            'Google authentication expired. Please re-connect in Settings.');
      }
